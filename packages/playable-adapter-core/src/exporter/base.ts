...
const globalReplacer = async (
  options: Pick<TBuilderOptions, "channel" | "zipRes" | "notZipRes"> & {
    $: CheerioAPI;
  }
) => {
  ...
  let splitSize = Number((MAX_ZIP_SIZE * 0.8).toFixed(0));
  ...
  // 不需压缩的文件
  $(`script[data-id="adapter-resource"]`).html(
    `window.__adapter_resource__=${JSON.stringify(notZipRes)};`
  );
};
...
export const exportSingleFile = async (
  ...
  const targetDir = createDir(join(getGlobalProjectBuildPath(), `${channel}`));
